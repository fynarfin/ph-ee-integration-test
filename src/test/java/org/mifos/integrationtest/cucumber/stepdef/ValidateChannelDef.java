    ...
    public void iCallThePostTransferAPIWithExpectedStatusOf(int expectedStatus) {
        await().atMost(awaitMost, SECONDS).pollDelay(pollDelay, SECONDS).pollInterval(pollInterval, SECONDS).untilAsserted(() -> {
            ...
            scenarioScopeState.response = RestAssured.given(requestSpec).header("Content-Type", "application/json")
                    ...
                    .baseUri(channelConnectorConfig.channelConnectorContactPoint).body(scenarioScopeState.createTransactionChannelRequestBody).expect()
    ...
    public void iCallTheGsmaTransactionAPIWithExpectedStatusOf(int expectedStatus) {
        await().atMost(awaitMost, SECONDS).pollDelay(pollDelay, SECONDS).pollInterval(pollInterval, SECONDS).untilAsserted(() -> {
            ...
            scenarioScopeState.response = RestAssured.given(requestSpec).header("Content-Type", "application/json")
                    ...
                    .header("amsName", scenarioScopeState.amsName)
                    .header("X-CallbackURL", scenarioScopeState.callbackUrl)
                    .baseUri(channelConnectorConfig.channelConnectorContactPoint).body(scenarioScopeState.createGsmaTransferRequestBody).expect()
                    .spec(new ResponseSpecBuilder().expectStatusCode(expectedStatus).build()).when()
